(ns nature-of-code.09-the-evolution-of-code.exercise-09-07
...
(defn fitness [target genes]
  (let [number-of-correct-characters
        (loop [idx 0
               score 0]
          (if (and (< idx (count genes)) (< idx (count target)))
            ...
            score))]
    (q/map-range
     (if (> number-of-correct-characters 0)
       (q/pow 2 number-of-correct-characters)
       0)
     0 (q/pow 2 (count target))
     0 1)))

(defn mating [population]
  (for [{:keys [fitness] :as dna} population
        n (range (int (q/ceil (* 100 fitness))))]
      dna))
